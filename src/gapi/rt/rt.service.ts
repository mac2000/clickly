...
const prependRtIfNeeded = (input: string) => input.indexOf('ga:') === 0
  ? input
  : `rt:${input}`;
...
const mapGaRequest = (input: Request): RawRequest => {
  ...
  if (input.sort && input.sort.length) {
    output.sort = input.sort
      .map(sort => `${sort.desc ? '-' : ''}${prependRtIfNeeded(sort.name)}`)
      .join(',');
  }
...
export class RtService {
  ...
  private async retrieveAsync(params: any) {
    ...
    do {
      const response = await this.gapi.client.analytics.data.realtime.get(Object.assign({}, mapGaRequest(params), {
